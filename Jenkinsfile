        ...
        steps{
          script {
            sshagent(credentials : ['JCI_SSH_KEY']) {
              ...
              //telegramSend(message: msg)
              commandStringChangeTag = "sed -i 's/tag:.*/tag: ${env.IMAGE_TAG}/g' /home/deploy/emirex-preprod/config/environments/preprod/testfront.yml"
              //echo commandStringChangeTag
              sh "ssh -o StrictHostKeyChecking=no Jenkins@35.205.47.217 /bin/bash -c sudo -u deploy '\"${commandStringChangeTag}\"'"
