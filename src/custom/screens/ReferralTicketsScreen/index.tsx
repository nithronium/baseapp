...
class ReferralTickets extends React.Component<Props> {
    ...
    public componentDidMount() {
        ...
        Promise.all([getReferralTickets(query), getOverall(), getBonusTickets()])
            .then(values => {
                const { direct, overall } = values[1];
                const bonuses = values[2];
                const referrals = values[0].list;
                const count = values[0].overall.count;
                const countActive = values[0].overall.countActive;
                const L2countActive = values[0].overall.L2countActive;
            ...
            <div className="pg-referral-tickets">
                <div className="top-holder">
                    <section id="top">
                        <ReferralBallance
                            ...
                            totalTickets={this.getTotalTickets(this.state.overall)}
                        >
                            <CardUser
                                message={this.props.intl.formatMessage}
                                title="Direct"
                                context={this.state.direct}
                                overall={this.state.overall.direct}
                                link="#direct"
                            />
                            <CardReferrals
                                message={this.props.intl.formatMessage}
                            ...
                            <CardBonuses
                                ...
                                overall={this.state.overall.bonuses}
                                link="#bonus"
                            />
                        </ReferralBallance>
                    </section>
                ...
                <section id="referral">
                    <ReferralTicketDetails
                        ...
                        L2countActive={this.state.L2countActive}
                ...
                <section id="bonus">
                    <BonusTicketDetails message={this.props.intl.formatMessage} context={this.state.bonuses} />
                </section>
                <section id="slider">
...
export const ReferralTicketsScreen = injectIntl(connect(mapStateToProps)(ReferralTickets));
